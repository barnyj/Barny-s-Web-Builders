...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2) Build the messages array (we'll cast it below)
    const rawMessages: { role: string; content: string }[] = [
      ...
      // 3) Include any prior turns (if provided)
      ...(history || []),
      // 4) Append the latest user message at the end
      { role: "user", content: message },
    ];

    // 5) Cast to the exact OpenAI type
    const messages = rawMessages as ChatCompletionMessageParam[];

    // 6) Call GPT-4 with our assembled messages
    ...
    // 7) Extract the assistant’s reply
